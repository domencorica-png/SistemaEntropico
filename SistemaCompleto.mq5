...
input bool     EnableAdaptiveTP = false;            // Attiva Adaptive Take Profit
input double   AdaptiveTP_Long_A = 1.2;             // Long - Coefficiente A (EMA ratio)
input double   AdaptiveTP_Long_B = 0.8;             // Long - Coefficiente B (offset)
input double   AdaptiveTP_Short_A = 1.3;            // Short - Coefficiente A (EMA ratio)
input double   AdaptiveTP_Short_B = 0.7;            // Short - Coefficiente B (offset)
input double   AdaptiveTP_Cap_Percent = 0.65;       // CAP massimo (%)
input double   AdaptiveTP_Min_Threshold = 0.2;      // Soglia minima (%)
...
int OnInit()
{
    ...
    if(EnableAdaptiveTP)
    {
        ...
        tpConfig.long_a = AdaptiveTP_Long_A;
        tpConfig.long_b = AdaptiveTP_Long_B;
        tpConfig.short_a = AdaptiveTP_Short_A;
        tpConfig.short_b = AdaptiveTP_Short_B;
        tpConfig.fixed_cap_percent = AdaptiveTP_Cap_Percent;
        tpConfig.min_threshold_percent = AdaptiveTP_Min_Threshold;
        tpConfig.fallback_percent = AdaptiveTP_Fallback;
        
        tradingWrapper.ConfigureAdaptiveTP(tpConfig);
        Print("Adaptive Take Profit ATTIVATO");
    }
