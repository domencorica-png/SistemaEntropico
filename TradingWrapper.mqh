...
private:
    ...
    datetime m_lastEntropyBarTime;     // Tempo dell'ultima barra processata dal filtro entropico
...
private:
    ...
    // Aggiorna il filtro entropico - OTTIMIZZATO: aggiorna solo su nuova barra
    void UpdateEntropyFilter()
    {
        if(!m_enableEntropyFilter) return;

        // Ottimizzazione: aggiorna solo quando c'è una nuova barra
        // Questo migliora drasticamente le performance in backtesting
        datetime current_bar_time = iTime(m_symbol, m_timeframe, 0);

        if(current_bar_time != m_lastEntropyBarTime)
        {
            m_entropyFilter.Update();
            m_lastEntropyBarTime = current_bar_time;
            m_lastFilterUpdate = TimeCurrent();
        }
    }
